...
// CONCATENATED MODULE: ./src/components/vue-cal/index.vue?vue&type=template&id=5e86a1d4&lang=pug&
...
var event_utils_createAnEvent = function createAnEvent(formattedDate, startTimeMinutes, vuecal) {
  var hours = parse_int_default()(startTimeMinutes / 60);

  var minutes = parse_int_default()(startTimeMinutes % 60);

  var endTimeMinutes = startTimeMinutes + 120;

  var event = _objectSpread({}, eventDefaults, {
    eid: "".concat(vuecal._uid, "_").concat(vuecal.eventIdIncrement++),
    start: formattedDate + (vuecal.time ? "".concat(hours, ":").concat(minutes) : ''),
    startDate: formattedDate,
    startTime: vuecal.time ? "".concat(hours, ":").concat(minutes) : null,
    startTimeMinutes: startTimeMinutes,
    end: formattedDate + (vuecal.time ? "".concat(hours + 2, ":").concat(minutes) : ''),
    endDate: formattedDate,
    endTime: vuecal.time ? "".concat(hours + 2, ":").concat(minutes) : null,
  ...
  methods: {
    ...
    next: function next() {
      ...
      switch (this.viewProps.view.id) {
        ...
        case 'week':
          var firstDayOfNextWeek = getPreviousFirstDayOfWeek(this.viewProps.view.startDate, this.startWeekOnSunday).addDays(7);
...
var header_component = normalizeComponent(
  ...
  headervue_type_template_id_1973c99c_lang_pug_render,
  headervue_type_template_id_1973c99c_lang_pug_staticRenderFns,
  false,
  null,
  null,
  null
...
var event_component = normalizeComponent(
  ...
  eventvue_type_template_id_108c4b94_lang_pug_render,
  eventvue_type_template_id_108c4b94_lang_pug_staticRenderFns,
  false,
  null,
  null,
  null
...
/* harmony default export */ var vue_calvue_type_script_lang_js_ = ({
  ...
  methods: {
    ...
    onCellMouseDown: function onCellMouseDown(e, cell) {
      var _this4 = this;
      ...
      if (this.editableEvents && !this.isDOMElementAnEvent(e.target) && ['month', 'week', 'day'].indexOf(this.view.id) > -1) {
        clickHoldACell.cellId = "".concat(this._uid, "_").concat(cell.formattedDate);
        clickHoldACell.timeoutId = setTimeout(function () {
          if (clickHoldACell.cellId) _this4.createAnEvent(cell, e);
        }, clickHoldACell.timeout);
      }
    },
    onCellTouchStart: function onCellTouchStart(e, cell) {
    ...
    onEventTitleBlur: function onEventTitleBlur(e, event) {
      ...
      if (event.linked.daysCount) {
        event.linked.forEach(function (e) {
          var dayToModify = _this5.mutableEvents[e.date];
    ...
    updateMutableEvents: function updateMutableEvents() {
      ...
      this.events.map(function (event) {
        ...
        var eid = "".concat(_this6._uid, "_").concat(_this6.eventIdIncrement++);
        event = assign_default()(_objectSpread({}, eventDefaults, {
          eid: eid,
          overlapped: {},
          overlapping: {},
          simultaneous: {},
        ...
        if (!(event.startDate in _this6.mutableEvents)) _this6.$set(_this6.mutableEvents, event.startDate, []); // eslint-disable-next-line

        _this6.mutableEvents[event.startDate].push(event);
        ...
        if (multipleDays) {
          (function () {
            ...
            var startDateFormatted = formatDate(startDate, 'yyyy-mm-dd', _this6.texts); // Update First day event.
            ...
            for (var i = 1; i <= datesDiff; i++) {
              var date = formatDate(new Date(startDate).addDays(i), 'yyyy-mm-dd', _this6.texts);
              eventPieces.push({
                eid: "".concat(_this6._uid, "_").concat(_this6.eventIdIncrement++),
                date: date
              });
            }
            ...
            var _loop = function _loop(_i2) {
              ...
              if (!(date in _this6.mutableEvents)) _this6.$set(_this6.mutableEvents, date, []);

              _this6.mutableEvents[date].push(_objectSpread({}, event, {
    ...
    createAnEvent: function createAnEvent(cell, e) {
      var mouseY = this.getPosition(e).y;
      var startTimeMinutes = mouseY * 60 / parse_int_default()(this.timeCellHeight) + this.timeFrom;

      event_utils_createAnEvent(cell.formattedDate, startTimeMinutes, this);
    },
